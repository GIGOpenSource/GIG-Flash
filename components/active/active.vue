<template>
	<z-paging ref="pagingRef" v-model="dataList" @query="queryList">
		<view v-for="(item, index) in isList ? list : list.slice(0, 1)" :key="index" class="con" @click="uni.navigateTo({ url: '/pages/community/details' })">
			<view class="top">
				<view class="left">
					<view class="" @click.stop="topath"><up-avatar :src="src" size="40"></up-avatar></view>
					<view class="message">
						<text>mask</text>
				...
				<view class="follow">
					<text v-if="isfollow && item.isFollow == 1" @click.stop="follow(index)">关注</text>
			...
			<view class="images">
				<image src="/static/tsp-icon/touxiang.jpg" mode="" @click="previewImage"></image>
			</view>
			<view class="bottom">
				<view class="">
				...
				<view class="right">
					<view class="" @click.stop="give(index)">
						<up-icon :name="item.flag ? 'thumb-up-fill' : 'thumb-up'" :color="item.flag ? '#ff0000' : '#ffffff'" size="26"></up-icon>
