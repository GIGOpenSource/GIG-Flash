<template>
	<view>
		<view class="comment-list-item f"  :ref="'commentId'+ index" @touchstart="textViewStart($event, item, index)" @touchmove="textViewMove($event, item, index)"
		 @touchend="textViewEnd($event, item, index)" @longpress="longpressText($event, item, index)" :class="[item.selectedRow?'list-item-active':'']">
			<view>
				<image class="comment-list-item-avatar" :src="item.avatar" mode="aspectFill"></image>
			</view>
			<view class="comment-list-item-right" :style="{width: screenWidth - 80 +'px'}">
				<view class="f f-v-c" @click="openPlReply('pl',item, index)">
					<text class="comment-list-item-title">{{item.username}}</text>
				...
				<view class="mgt7" :class="[item.foldShow?'comment-list-item-list' : '']">
					...
					<view class="mgt7 f f-v-c" v-if="item.imgList && item.imgList.length > 1">
						<view :class="[imgIdx == 1 ? 'gallery-spacing':'']" v-for="(itemImg,imgIdx) in item.imgList" :key="imgIdx"
						 :style="{width: galleryPlwidth+'px',height: galleryPlwidth+'px', position: 'relative'}" @click="previewImg(item.imgList,imgIdx)">
							<image class="comment-list-item-imgBox-img" :src="itemImg" mode="aspectFill" v-if="imgIdx < 3"
					...
					<view class="mgt7 f f-v-c" v-if="item.imgList && item.imgList.length == 1">
						<view v-for="(itemImg,imgIdx) in item.imgList" :key="imgIdx" @click="previewImg(item.imgList,imgIdx)">
							<view :class="[!item.imgWidth?'comment-list-item-list' : '']">
								<image class="comment-list-item-imgBox-img" :src="itemImg" mode="aspectFill" @load="loadImage($event, 'pl', item, index)"
				...
				<view class="f f-lv-s" :class="[item.foldShow ? '':'mgt7']">
					...
					<view class="f f-v-c">
						<view class="f f-v-c comment-list-item-dzmgr" @click="plLikeBtn('pl', item, index)">
							<view class="comment-list-item-dzImg" :class="[item.likeeffect == true ? 'fabulous-taoxin': item.likeeffect == false ? 'cancel-taoxin' : '']"
							 :ref="'likePlRef'+index">
							...
							<text class="comment-list-item-dzNum" v-if="item.likes">{{item.likes}}</text>
						</view>
		...
		<view class="f contBox">
			...
			<view class="mgl5">
				...
				<view v-if="item.listTotal > 0">
					<view v-for="(it,i) in item.list" :key="i">
						...
						<view v-if="it.expand">
							...
							<view class="f contBox-item" v-for="(v,idx) in it.list" :key="idx" :ref="'commentId'+ index + i + idx" :id="'commentId'+ index + i + idx"
							 :class="[v.show?'mgb5' : 'comment-list-item-list', v.selectedRow?'list-item-active':'']" :style="{width: screenWidth - 60 +'px'}"
							 @touchstart="textViewStart($event, item, index, i ,idx)" @touchmove="textViewMove($event, item, index, i ,idx)" @touchend="textViewEnd($event, item, index, i ,idx)"
							 @longpress="longpressText($event, item, index, i ,idx)">
								...
								<view class="comment-list-item-right">
									<view class="f f-v-c" @click="openHfReply('hf',item, v, index, i, idx)">
										<text class="comment-list-item-title">{{v.username}}</text>
									...
									<view class="mgt7" :class="[v.foldShow?'comment-list-item-list' : '']">
										<!-- <text class="comment-list-item-contText" :style="{width: screenWidth - 110 +'px'}">{{v.content}}</text> -->
										
										<!-- 文本内容 -->
										<view :style="{width: screenWidth - 120 +'px'}">
											<commentText v-if="v.textShow" :index="index" :i="i" :idx="idx" :item="v" showType="hf" :conText="v.content" @packUpText="packUpText"></commentText>
										</view>
										
										<!-- 图集 -->
										<view class="mgt7 f f-v-c" v-if="v.imgList && v.imgList.length > 1">
											<view :class="[imgIdx == 1 ? 'gallery-spacing':'']" v-for="(itemImg,imgIdx) in v.imgList" :key="imgIdx"
											  :style="{width: galleryHfwidth+'px',height: galleryHfwidth+'px', position: 'relative'}" @click="previewImg(v.imgList,imgIdx)">
												<image class="comment-list-item-imgBox-img" :src="itemImg" mode="aspectFill" v-if="imgIdx < 3"
												 :style="{width: galleryHfwidth+'px',height: galleryHfwidth+'px'}"></image>
										...
										<view class="mgt7 f f-v-c" v-if="v.imgList && v.imgList.length == 1">
											<view v-for="(itemImg,imgIdx) in v.imgList" :key="imgIdx" @click="previewImg(v.imgList,imgIdx)">
												<view :class="[!v.imgWidth?'comment-list-item-list' : '']">
													<image class="comment-list-item-imgBox-img" :src="itemImg" mode="aspectFill" @load="loadImage($event, 'hf', v, index, i, idx)"
													 :style="{width: v.imgStyle.width,height: v.imgStyle.height}"></image>
												</view>
											</view>
										</view>
										<!-- 表情 -->
										<view class="mgt7 f f-v-c" v-if="v.phizImg" @click="previewImg([v.phizImg],0)">
											<image class="comment-list-item-bqImg" :class="[screenWidth > 500 ? 'bqImgWhPx':'bqImgWhRpx']" :src="v.phizImg" mode="aspectFill"></image>
										</view>
									</view>
									<view class="f f-lv-s" :class="[v.foldShow ? '':'mgt7']">
										...
										<view class="f f-v-c" v-else @click="openHfReply('hf',item, v, index, i, idx)">
											...
											<text class="comment-list-item-fh">·</text>
											<text class="comment-list-item-zbdz">{{v.ipAddres}}</text>
											<text class="comment-list-item-reply">回复</text>
										</view>
										<view class="f f-v-c">
											<view class="f f-v-c comment-list-item-dzmgr" @click="plLikeBtn('hf', v, index, i, idx)">
												<view class="comment-list-item-dzImg" :ref="'likeHfRef'+index+i+idx" :class="[v.likeeffect == true ? 'fabulous-taoxin': v.likeeffect == false ? 'cancel-taoxin' : '']">
				...
				<view class="f f-v-c mgt5" v-if="item.listTotal > 0 && !item.loadShow">
					<view>
						<view class="f f-v-c" v-if="item.expandNum < item.listTotal" @click="openUpComment(item,index)">
							<view class="comment-list-item-zkhfFg"></view>
							<text class="comment-list-item-zkhfText"  v-if="item.expandNum > 0">展开更多</text>
							<text class="comment-list-item-zkhfText" v-else>展开{{item.listTotal}}条回复</text>
							<view class="comment-list-item-zkhfZk">
								<image src="/static/tsp-icon/tsp-zk.png" class="comment-list-item-zkhfZk"></image>
							</view>
						</view>
					</view>
					<view>
						<view class="f f-v-c mgl20" v-if="item.expandNum > 0" @click="packUpComment(item,index)">
							<view class="comment-list-item-zkhfFg" v-if="item.expandNum >= item.listTotal"></view>
							<text class="comment-list-item-zkhfText">收起</text>
							<view class="comment-list-item-zkhfSq">
								<image src="/static/tsp-icon/tsp-zk.png" class="comment-list-item-zkhfSq"></image>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods:{
			...
			loadImage(event, type, item, index, i, idx){
				...
				if(objItem.imgWidth == objItem.imgHeight){ //方形图
					objItem.imgStyle = {width: this.screenWidth > 500 ? '160px' : '335rpx',height: this.screenWidth > 500 ? '160px' : '335rpx'}
				}else if(objItem.imgWidth > objItem.imgHeight){ //宽图
					objItem.imgStyle = {width: this.screenWidth > 500 ? '160px' : '335rpx',height: this.screenWidth > 500 ? '110px' : '242rpx'}
				}else{ //高图
					objItem.imgStyle = {width: this.screenWidth > 500 ? '126px' : '252rpx',height: this.screenWidth > 500 ? '166px' : '332rpx'}
				}
			...
			plLikeBtn(type, item, index, i, idx){
				...
				obj.likes = obj.likeShow ? obj.likes + 1 : obj.likes - 1
