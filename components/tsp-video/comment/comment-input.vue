<template>
	<view class="input-popup" v-if="modelValue" :style="{height: screenHeight + 'px'}">
		<!-- 遮罩层 -->
		<view class="input-mask" @click="close" :style="{width: screenWidth+'px',height: screenHeight+'px',opacity: showPopup ? 1 : 0,}"></view>
		
		<!-- 内容区域 -->
		<view class="input-content" :style="{width: screenWidth+'px',opacity: showPopup ? 1 : 0}" @click="clickStop">
			<view class="input-content-top">
				<view class="input-content-top-box" :style="{width: screenWidth - 30 +'px'}">
					<view :style="{width: screenWidth - 56 +'px',minHeight: '40px'}">
						<textarea :placeholder="placeholderText" class="input-content-top-textarea" v-model="remarkText" :adjust-position="false" :focus="isFocus" 
						 auto-height :maxlength="-1" v-if="showText" :style="{width: screenWidth - 56 +'px',minHeight: '40px',maxHeight: '100px'}"
						 @touchstart="closeBrow"/>
					</view>
					<!-- 表情图 -->
					<view class="input-content-top-box-bqtp" v-if="phizImg">
						<image :src="phizImg" class="input-content-top-box-bqtp-img" @click="openPreview([phizImg])"></image>
						<view class="input-content-top-box-del f f-lv-c"  @touchstart="phizImg = ''">
							<image src="/static/tsp-icon/tsp-del.png" class="input-content-top-box-del-img"></image>
						</view>
					</view>
					<!-- 图片或者图集 -->
					<scroll-view class="f" scroll-x v-if="plImgList.length > 0" :style="{width: screenWidth - 56 +'px'}" :show-scrollbar="false">
						<view class="f f-v-c">
							<view class="input-content-top-box-bqtp mgr10" v-for="(item,index) in plImgList" :key="index">
								<image :src="item" class="input-content-top-box-bqtp-img" @click="openPreview(plImgList,index)"></image>
								<view class="input-content-top-box-del f f-lv-c"  @touchstart="deletePlImg(index)">
									<image src="/static/tsp-icon/tsp-del.png" class="input-content-top-box-del-img"></image>
								</view>
							</view>
							<view class="input-content-top-box-addImg f f-lv-c" v-if="plImgList.length < 9" @touchstart="openChooseImage(1)">
								<image src="/static/tsp-icon/tsp-tj.png" class="input-content-top-box-addImg-img"></image>
							</view>
						</view>
					</scroll-view>
				</view>
			...
			<view class="f f-lv-s input-operate">
				<view class="f f-v-c">
					<view class="input-operate-img mgl7" @touchstart="openChooseImage(0)">
						<image src="/static/tsp-icon/upImg.png" class="input-operate-icon"></image>
					</view>
					<text class="input-operate-at">@</text>
					<view class="input-operate-img" @touchstart="changeBrow">
						<image src="/static/tsp-icon/tsp-jianpang.png" class="input-operate-icon" v-if="showBrow"></image>
						<image src="/static/tsp-icon/biaoqing.png" class="input-operate-icon" v-else></image>
					</view>
				</view>
				<view class="input-operate-send f f-lv-c" :class="{sendActive: remarkText || phizImg || plImgList.length > 0}" @click="sendBtn">
					<text class="input-operate-send-text" :class="{sendActive: remarkText || phizImg || plImgList.length > 0}">发送</text>
				</view>
			</view>
			
			<!-- 表情菜单栏 -->
			<view :style="{width: screenWidth+'px',height: '45px',overflow: 'hidden',position: 'relative'}">
				<view class="input-phiz f f-lv-s" :style="{width: screenWidth+'px',left: !showBrow ? 0 : -screenWidth+'px'}">
					<text class="input-phiz-text" v-for="(item,index) in emojiRecommend" :key="index" @click="addTextEmoji(item)">{{item.emoji}}</text>
				</view>
				<scroll-view class="input-tool f" scroll-x :style="{width: screenWidth+'px',left: showBrow ? 0 : -screenWidth+'px'}" :show-scrollbar="false">
					<view class="f f-v-c" style="height: 45px;">
						<view v-for="(item,index) in browList" :key="index">
							<view class="input-tool-item f f-lv-c"
							 :class="[browIndex == index ? 'itemActive':'']" @click="openTool(index)">
			...
			<!-- 表情部分 -->
			<view :style="{width: screenWidth+'px',height: swHeight+'px',overflow: 'hidden',position: 'relative'}">
				<swiper class="input-swiper" :current="browIndex" :duration="300" @change="onswiperchange"
				 :style="{width: screenWidth+'px',height: swHeight+'px',left: showBrow ? 0 : -screenWidth+'px'}">
					<swiper-item v-for="(item,index) in browList" :key="index">
						<scroll-view class="input-swiper-item" scroll-y :style="{height: swHeight+'px'}">
							<view style="height: 10px;"></view>
							<!-- emoji表情 -->
							<view class="f fw" v-if="item.type == 'emoji'">
								<view class="input-swiper-item-bqtb" v-for="(em,eid) in emojiList" :key="eid"
								  :style="{width: emojiWidht+'px',height: emojiWidht+'px'}" @click="addTextEmoji(em)">
									<text class="input-swiper-item-bqtb-text">{{em.emoji}}</text>
								</view>
							</view>
							<!-- 表情包 -->
							<view class="f fw" v-if="item.type !== 'emoji'">
								<!-- 添加按钮 -->
								<view class="input-swiper-item-addbq f f-lv-c" v-if="item.type == 'scbq'"
								 :style="{width: browWidht+'px',height: browWidht+'px'}">
									<image src="/static/tsp-icon/tsp-tj.png" :style="{width: browWidht / 2.5 +'px',height: browWidht / 2.5 +'px'}"></image>
								</view>
								<!-- 搜索按钮 -->
								<view class="input-swiper-item-addbq f f-lv-c" v-if="item.type == 'gif'"
								 :style="{width: browWidht+'px',height: browWidht+'px'}">
									<image src="/static/tsp-icon/tsp-search.png" :style="{width: browWidht / 2.5 +'px',height: browWidht / 2.5 +'px'}"></image>
								</view>
								<view class="f f-lv-c input-swiper-item-bqImg" v-for="(v,idx) in item.list" :key="idx"
								 :style="{width: browWidht+'px',height: browWidht+'px'}" @click="addImgPhiz(v)">
									<image :src="v.imgUrl" mode="widthFix" :style="{width: browWidht+'px',height: browWidht+'px'}"></image>
								</view>
							</view>
	...
	export default {
		...
		created() {
			...
			uni.onKeyboardHeightChange(res => {
				...
				if(this.boardHeight == 0 && !this.showBrow && this.openType != 'img'){
					...
					this.swHeight = 0
					// #endif
				}else if(this.boardHeight == 0 && !this.showBrow && this.openType == 'img'){
		...
		methods: {
			...
			close() {
				...
				this.browIndex = 0
				uni.hideKeyboard()
				let data = {text: this.remarkText, phizImg: this.phizImg, sureSend: this.sureSend, plImgList: this.plImgList}
				/* 回复他人 */
				if(this.currReplyInfo.replyType){
