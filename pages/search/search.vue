	...
	<z-paging ref="paging" v-model="dataList" @query="queryList">
		<template #top>
			...
			<up-navbar bgColor="transparent" leftIconColor="#fff" placeholder autoBack :fixed="false">
				<template #right>
					<view class="search">
						<up-input placeholder="输入搜索内容" prefixIcon="search" shape="circle" border="none"
							style="height: 28px;" prefixIconStyle="font-size: 22px;color: #fff" color="#fff"
							v-model="searchText">
							<template #suffix>
				...
				<view class="tags_box">
					<view class="tag_list" v-for="(item,i) in tagsList" :key="i">
						...
						<view class="lebel_list">
							<view :class="['item',sub?.checked?'active':'']" v-for="(sub,j) in item.children" :key='j'
...
<script setup>
	...
	const handleClickSearch = () => {
		if (searchText.value && searchHistroyList.value.indexOf(searchText.value) == -1) searchHistroyList.value
			.unshift(searchText.value)
